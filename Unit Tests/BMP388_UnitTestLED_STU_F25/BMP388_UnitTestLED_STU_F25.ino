...
void setup() {
  ...
  Serial.println("Feather RP2040 - BMP388 Unit LED Test");
  ...
  if (!SD.begin(config)) {      // Try to initialize the SD card with given config
    ...
    digitalWrite(***Fill_In_Here***, HIGH);  // Turn on red LED for SD card failure
  ...
  if (dataFile) {
    ...
    digitalWrite(***Fill_In_Here***, ***Fill_In_Here***); // Turn the red pin on 
    while (1);  // Halt program execution
  }
  ...
  if (!bmp.begin_I2C()) {           // Initialize BMP388 sensor over I2C
    ...
    digitalWrite(***Fill_In_Here***, ***Fill_In_Here***);     // Turn on Red LED if BMP388 sensor could not be found
...
void loop() {
  ...
  if (sdAvailable()) {
    ...
    if (bmp.performReading()) {
      ...
      digitalWrite(***Fill_In_Here***, ***Fill_In_Here***); // Ensure green LED is off
      digitalWrite(***Fill_In_Here***, ***Fill_In_Here***); // Make sure red LED is on
      while(1);
    }
